...
module.exports = function (context) {
    ...
    // 参考
    // http://www.benricho.org/kanji/kyoikukanji/check-jyoyo-kanji.html
    // http://nanto.asablo.jp/blog/2015/12/31/7966713
    ...
    // http://nanto.asablo.jp/blog/2015/12/31/7966713
    var regex = /(?:[\u2E80-\u2E99\u2E9B-\u2EF3\u2F00-\u2FD5\u3005\u3007\u3021-\u3029\u3038-\u303B\u3400-\u4DB5\u4E00-\u9FCC\uF900-\uFA6D\uFA70-\uFAD9]|[\uD840-\uD868][\uDC00-\uDFFF]|\uD869[\uDC00-\uDED6\uDF00-\uDFFF]|[\uD86A-\uD86C][\uDC00-\uDFFF]|\uD86D[\uDC00-\uDF34\uDF40-\uDFFF]|\uD86E[\uDC00-\uDC1D]|\uD87E[\uDC00-\uDE1D])/g;

    return _defineProperty({}, Syntax.Str, function (node) {
        // "Str" node
        var text = getSource(node); // Get text

        var kanjiAry = text.match(regex);

        if (!kanjiAry) {
            return;
        }

        for (var i = 0; i < kanjiAry.length; i++) {
            if (joyoKanji.indexOf(kanjiAry[i]) == -1) {
                var indexOfBugs = text.indexOf(kanjiAry[i]);
                var ruleError = new RuleError('「' + kanjiAry[i] + "」は常用漢字ではありません。", {
