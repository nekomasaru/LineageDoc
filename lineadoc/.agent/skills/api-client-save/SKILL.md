...
- **バージョン作成の省略**: 保存ごとに必ず `versions` にレコードを追加。ただし、内容に変更がない場合はスキップすることを推奨。

# 保存の最適化（重複排除）

履歴ツリーの肥大化を防ぐため、以下のロジックで保存を制御すること。

1. **内容比較**: 現在のエディタの内容と、直近の履歴（`latestEvent.content`）を比較する。
2. **スキップ条件**: 内容が完全に一致し、かつ「ブランチ作成」や「明示的な理由付き保存」でない場合は、新しい `Version` レコードの作成をスキップする。
3. **メタデータ更新**: 文書自体の `updated_at` や `title` の更新は、内容の変化に関わらず実行してよい。
