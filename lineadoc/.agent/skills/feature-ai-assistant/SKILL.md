...
4. `AIChatPane.tsx` のコマンドバーに「AI指示（Modal）」と「マイルストーン保存（Toast+Save）」を統合。
5. `AISelectionTooltip.tsx` の該当するサブメニュー（Style, Analyze, Structure等）または `AIChatPane.tsx` のコマンドバーに反映。
...
- **Flexible Layout Management**: 
  - `isRightPanelPinned` (固定) および `isAiChatFullPage` (全画面) の状態に基づき、表示スタイルを動的に切り替える。
  - フロートモード時は `z-index` を適切に設定し、エディタの上に影付きのレイヤーとして重ねる。
- **Focused & Slim Design**: 
  - ヘッダー部分の冗長な情報を削ぎ落とし、チャットログの表示領域を1ピクセルでも多く確保する。
  - パディングやマージンはモードに応じて最適化（固定時は標準、フロート時は少し余裕を持たせるなど）。
  - パディングやマージンはモードに応じて最適化（固定時は標準、フロート時は少し余裕を持たせるなど）。
- **Safety First Operation**: 
  - 破壊的な操作（会話リセットなど）には必ず `ConfirmModal` を介し、ユーザーの明確な同意を得るプロセスを実装する。
