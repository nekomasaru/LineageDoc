...
- **History Tab (Linea)**:
    ...
    - **Interactive History**:
        ...
        - **Milestone Management**: 重要なバージョンを「マイルストーン」としてマーク。AI要約カードが自動生成され、ツリー上で強調表示される。
        - **Semantic Feed**: 手動保存（Draft）は視覚的にデエンファシス（opacity低下）され、マイルストーンが目立つように視覚マージンが調整される (ROW_HEIGHT=90px)。
...
- **Quality Tab (Governance)**:
    ...
    - 公文書ガイドラインへの準拠チェック。
- **LineaDoc AI Tab (✨)**:
    - **Focused Header**: 垂直方向のスペースを確保するため、パディングを `p-2 px-3` にスリム化。
    - **Display Toggles**: ヘッダー右側に「全画面（最大化）」「ピン留め（固定/解除）」のトグルアイコンを配置。
    - **Selection Context**: 現在選択中のテキストプレビューをコンパクトに表示。
    ...
    - **Reset Confirmation**: 会話リセット時は `ConfirmModal` で警告を表示（誤操作防止）。
    - **FullScreen Integration**: 全画面モードの開始/終了ボタンをAIチャットヘッダー内に統合し、現在のモードに関わらず常に同じ位置で操作可能にする。
