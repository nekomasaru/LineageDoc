...
### 1. Primary: Supabase pgvector (近景・高精度・権限重視)
- **役割**: 現在進行中のプロジェクト文書、最近の起案、組織の内規。
- **メリット**: **RLS (Row Level Security)** 連携により、ユーザーの権限外の文書参照を防止。

### 2. Secondary: Vertex AI Search (遠景・広範囲・アーカイブ)
- **役割**: 数年分の全庁的な過去文書、膨大な PDF アーカイブ、外部の公開資料。

### 3. UI/UX への統合 (情報の透明性)
- **Source Badges**: AI の回答内に、どの Layer/文書に基づいたかを示すバッジ（例: `[庁内内規]`, `[文書A]`）を表示。
