...
- **Unified Feedback Model**: `AGENTS.md` の「No Toast」ルールを厳格に適用。ボタンのラベル変化（例：「反映済み」）によるインライン・フィードバック。
- **Flexible Display Modes (NEW)**:
  - **サイドピーク（フロート）**: エディタを広々と使ったまま、パネルを上に重ねて表示。パネル外クリックで自動的に閉じる。
  - **ピン留め（固定）**: パネルをタイリング表示し、執筆しながら常時参照。エディタはパネル幅分だけ自動リサイズ。
  - **フォーカス（フルページ）**: 画面中央に最大化表示。長文の検討や構造化データの確認に集中。ヘッダーのトグルボタンで動的に切り替え。
- **Safety & Recovery**:
  - **Reset Confirmation**: 会話のリセットボタン（RefreshCcw）押下時、即座にメッセージを消去せず、`ConfirmModal` によるユーザー確認を必須とする。これにより誤操作による履歴消失を防止。
- **Slim UI Header**: 垂直方向の圧迫を最小化するため、ヘッダーパディングを縮小し、タイトル情報をスリム化（Focused Layout）。
